...
const ActionDropdown = ({
  ...
  const [name, setName] = useState((file as any).name);
    ...
    const actions = {
      ...
      delete: () =>
        deleteFile({
          fileId: file.$id,
          path,
          bucketFileId: (file as any).bucketFileId,
        }),
    };
      ...
      <DialogContent className="shad-dialog button">
        <DialogHeader className="flex flex-col gap-3">
          ...
          {action.value === 'details' && (
            <FileDetails file={file} owner={owner} />
          )}
          ...
          {action.value === 'delete' && (
            <p className="delete-confirmation">
              ...
              <br />
              <span className="delete-file-name">{(file as any).name}</span>?
            </p>
          )}
        </DialogHeader>
          ...
          {actionsDropdownItems.map((actionItem) => (
            ...
            >
              {actionItem.value === 'download' ? (
                <Link
                  href={constructDownloadUrl((file as any).bucketFileId)}
